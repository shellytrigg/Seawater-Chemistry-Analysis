...
salinity2 <- read_xlsx("Spec_Salinity Log_2017.xlsx")

#convert date column
salinity1$Date <- as.POSIXct(strptime(salinity1$Date, "%y-%m-%d"))
salinity2$Date <- as.POSIXct(strptime(salinity2$Date, "%Y-%m-%d"))

salinity1 <- salinity1[,which(names(salinity1) %in% c("Date", "Salinity"))]
salinity2 <- salinity2[,which(names(salinity2) %in% c("Date", "Salinity"))]
#combine individuals data files into one data frame
salinity <- rbind(salinity1,salinity2)
#remove rows where salinity is NA
salinity <- subset(salinity, !is.na(Salinity))
...
#made corrections to 1 Apr 2016.csv; last 5 entries had the incorrect date entered
#made corrections to 7 Apr 2016.csv; date originally entered as 4/6/2016
#made corrections to 26 Jul 2016.csv; date originally entered as 4/6/2016 except for first line

####manual salinity corrections to original excel files; used salinity log data to correct values that were not entered correctly in the Spec pH data files 
##made corrections to 5 Mar 2016.xls; salinity was entered as 29.60 (old default) and should be ~28.57 (03/04/16 log reading)
##made corrections to 8 Mar 2016.xls; salinity was entered as 29.60 (old default) and should be ~28.44 (03/09/16 log reading)
##made corrections to 11 Mar 2016.xls; salinity was entered as 29.60 (old default) and should be ~28.49 (03/11/16 log reading)
##made corrections to 14 Mar 2016.xls; salinity was entered as 29.60 (old default) and should be ~28.24 (03/14/16 log reading)
##made corrections to 26 Mar 2016.xls; salinity was entered as 29.60 (old default) and should be ~28.59 (03/26/16 log reading)
##made corrections to 4 Apr 2016.xls; salinity was entered as 29.60 (old default) and should be ~28.23 (04/05/16 log reading)
##made corrections to 11 Jun 2016.xls; salinity was entered as 28.27 (old default) and should be ~29.42 (06/10/16 log reading)
##made corrections to 14 Jun 2016.xls; salinity was entered as 28.27 (new default) and should be 29.40 (06/14/16 log reading)
##made corrections to 20 Jun 2016.xls; salinity was entered as 28.27 (new default) and should be 29.34 (06/20/16 log reading)
##made corrections to 17 Jul 2016.xls; salinity was entered as 28.27 (new default) and should be 29.65 (07/17/16 log reading)
##made corrections to 23 Jul 2016.xls; salinity was entered as 28.27 (new default) and should be 29.94 (07/25/16 log reading)
##made corrections to 26 Jul 2016.xls; salinity was entered as 32.03  and should be 30.03 (07/26/16 log reading)
##made corrections to 4 Aug 2016.xls; salinity was entered as 28.27 (new default) and should be 30.11 (08/04/16 log reading)
##made corrections to 12 Apr 2017.xls; salinity was entered as 28.27 (new default) and should be 27.86 (04/16/17 PMEL reading)
##made corrections to 8 Jun 2017.xls; salinity was entered as 28.27 (new default) and should be 28.42 (06/08/17 log reading)
##made corrections to 11 Jun 2017.xls; salinity was entered as 30.00 and 28.27 and should be 28.32 (06/11/17 log reading)
##made corrections to 2 Jul 2017.xls; salinity was entered as 28.27 (new default) and should be 29.18 (06/08/17 log reading)
##made corrections to 10 Aug 2017.xls; salinity was entered as 26.6 and should be 29.60 (08/10/17 log and PMEL reading)
#29 Jul 2016.xls contains 8 rows at the bottom that do not contain saliniy values or MOATS values
...
moatsNames <- as.numeric(seq(1,13))
...
#set working directory to master MOATS data folder containing all individual MOATS folders (i.e. M1-M13) each with a set of .lvm files
setwd("/Users/Shelly/Desktop/2016-17_CrabExp_WaterChem/data/MOATS/")
...
for(i in 1:length(folder.list)){
  folder.list2 <- dir(paste(getwd(), folder.list[i], sep = "/"))
  for(j in 1:length(folder.list2)){
    file.names <- dir(paste(getwd(), folder.list[i], folder.list2[j], sep = "/"), pattern = ".lvm")
    for(k in 1:length(file.names)){
      print(paste(folder.list[i], folder.list2[j],file.names[k], sep = "/"))
      #start pulling in the data.  
      #First count the max number of columns in the file
      ncols <- max(count.fields(paste(getwd(),folder.list[i], folder.list2[j],file.names[k],sep="/"), sep = "\t"))
      #Read in data to get date and time data were collected
      dtemp <- read.table(paste(getwd(),folder.list[i], folder.list2[j],file.names[k],sep="/"), sep="\t", skip = 9, nrows=2, header=FALSE, col.names = paste0("V", seq_len(ncols)), stringsAsFactors=FALSE, fill = TRUE)
      ...
      #convert to posix 
      datetime <- as.POSIXct(strptime(datetime, "%Y/%m/%d_%H:%M:%S"))
      #now read in the rest of the data
      dtemp <- read.table(paste(getwd(),folder.list[i], folder.list2[j],file.names[k],sep="/"), sep="\t", skip = 22, header=FALSE, col.names = paste0("V", seq_len(ncols)), stringsAsFactors=FALSE, fill = TRUE,skipNul = TRUE)
      ...
      dtemp$moats <- as.numeric(substr(folder.list2[j],2,3))
